...
import { AlertBox, CodeSnippet, Form, LearnMoreMessage, Modal } from '@/components';
...
export const SetupModal: React.FC<SetupModalProps> = ({ otpUrl, isLoading, ...dialogProps }) => {
...
const Step1: React.FC<Step1Props> = ({ otpUrl, handleSetRecoveryCodes, isLoading }) => {
  const [isUnableToScan, setIsUnableToScan] = useState<boolean>(false);
  const secretKeyField = Form.useField<string>('secretKey');

  const { shouldDisableSubmit, isSubmitting, submit } = Form.useContext<Create2FAFormValues, Create2FAFormResponse>();
  ...
  return (
    <>
      ...
      <Text>Follow the steps below to secure your account with two-factor authentication.</Text>
      <S.TextSection>
        <Text variant="primary" weight={700}>
          Scan the QR code
        </Text>
        <Text>Use an authenticator app to scan the code below.</Text>
        <S.QrContainer isLoading={isLoading}>{isLoading || !otpUrl ? <Icon name="spinner" /> : <S.QRCode value={otpUrl} />}</S.QrContainer>
      </S.TextSection>
      ...
      <LearnMoreMessage prefix="Need help? Follow the instructions" href={constants.EXTERNAL_LINK.FLEEK_DOCS_2FA_SETUP}>
        here
      </LearnMoreMessage>
      <Modal.CTARow>
...
const Step2: React.FC<Step2Props> = ({ recoveryCodes, isLoading }) => {
  ...
  return (
    <>
      ...
      />
      <LearnMoreMessage prefix="Need Help?" href={constants.EXTERNAL_LINK.FLEEK_DOCS_2FA_RECOVERY_CODES}>
      ...
      <Modal.CTARow>
        ...
        <Dialog.Close asChild>
          <Button disabled={!hasDownloadedCodes} onClick={reset} className="flex-1">
            I saved my codes
          </Button>
        </Dialog.Close>
      </Modal.CTARow>
