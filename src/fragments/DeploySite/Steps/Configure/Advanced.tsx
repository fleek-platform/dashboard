...
export const Advanced: React.FC = () => {
  ...
  const { sourceProvider, gitBranch, gitRepository, gitUser, accessToken } = useDeploySiteContext();
...
const EnvironmentVariable: React.FC<EnvironmentVariableProps> = ({
  ...
  return (
    <S.Advanced.Row key={index}>
      <FormField.Root error={Boolean(envVarNameError) && shouldValidate} className="flex-1">
        <Input.Root>
          <Input.Field placeholder="EXAMPLE_NAME" value={key} onChange={handleEnvNameChange} onBlur={handleEnvNameValidation} />
        </Input.Root>
        {envVarNameError && shouldValidate && <FormField.Hint>{envVarNameError}</FormField.Hint>}
      </FormField.Root>

      <FormField.Root error={Boolean(envVarValueError) && shouldValidate} className="flex-1">
        <Input.Root>
          <Input.Field
        ...
        {envVarValueError && shouldValidate && <FormField.Hint>{envVarValueError}</FormField.Hint>}
      </FormField.Root>
