...
export const Filecoin: React.FC<FilecoinProps> = ({ dealID }) => {
  ...
  return (
    <BoxWithFooter footerIcon="filecoin" footerIconLabel="Filecoin" footer="The hash for your site content" iconContainerVariant="filecoin">
      <S.Container>
        <S.Header>
          <Text as="h3" variant="primary" size="lg" weight={700}>
            Filecoin
          </Text>
          <BadgeText colorScheme={hasDealId ? 'yellow' : 'amber'}>{hasDealId ? 'Backed Up' : 'Syncing'}</BadgeText>
        </S.Header>
        <S.Main>
