...
const migrationStatusMap: Record<MigrationStatusBoxProps['status'], { text: string; className: string }> = {
  ...
  PARTIALLY_COMPLETE: { text: 'Partially Complete', className: 'text-success-11' },
  FAILED: { text: 'Failed', className: 'text-danger-11' },
};

export const MigrationStatusBox: React.FC<MigrationStatusBoxProps> = ({ status, date, triggeredBy }) => {
  return (
    <S.Content.MigrationStatus.Container>
      <S.Content.MigrationStatus.Row>
        <Text>Status</Text>
        <Text variant="primary" weight={500} className={migrationStatusMap[status].className}>
