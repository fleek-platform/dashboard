...
export const TemplateCard = forwardStyledRef<HTMLAnchorElement, TemplateCardProps>(Card.Root, ({ isLoading, template, ...props }, ref) => {
  ...
  return (
    <LinkBox {...props} ref={ref} href={routes.template.indexed({ templateId: template.id! })} className="p-0 gap-0 bg-transparent">
      <Card.Cover src={template.banner} className="aspect-[16/9] max-h-[8rem] object-cover" />
      <Box className="p-4">
        <Box className="flex-row justify-between">
          <Text as="h3" variant="primary" size="md" weight={700}>
            {template.name}
          </Text>
          <Image src={template.framework?.avatar} alt={template.framework?.name} className="size-[1.75em] rounded bg-neutral-2">
            <Icon name="gear" />
          </Image>
        </Box>
...
const AuthorAvatar: React.FC<AuthorAvatarProps> = ({ creator = {} as AuthorAvatarProps['creator'] }) => {
  ...
  return (
    <Box className={cn('flex-row gap-2 items-center', { 'bg-monochrome-reverse': isFleekAuthored })}>
      <Avatar enableIcon icon={isFleekAuthored ? 'fleek' : 'person'} src={creator.avatar} className="size-6" />
