import { createTemplateSchema, updateTemplateSchema } from '@fleek-platform/utils-validation';
import React, { useEffect, useState } from 'react';
import { useClient } from 'urql';

import { BannerField, Form, SettingsModal } from '@/components';
import { useTemplateQuery, useUpdateTemplateMutation } from '@/generated/graphqlClient';
import { useToast } from '@/hooks/useToast';
import { ChildrenProps } from '@/types/Props';
import { Button, Text } from '@/ui';

export type TemplateUpdateModalProps = ChildrenProps<{ templateId: string }> & Omit<React.ComponentProps<typeof SettingsModal>, 'trigger'>;

export const TemplateUpdateModal: React.FC<TemplateUpdateModalProps> = ({ children, templateId, onOpenChange, open, ...props }) => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (!updateTemplateResult.data) {
          throw updateTemplateResult.error || new Error('Unable to update template');
        }
  ...
  return (
    <Form.Provider value={form}>
      <SettingsModal trigger={children} {...props} open={isOpen} onOpenChange={handleOpenChange}>
        <SettingsModal.Heading>Edit Template Details</SettingsModal.Heading>

        <Text>Edit the details for your Template, these will be used in the Fleek Template Gallery.</Text>

        <Form.InputField name="name" placeholder="Template" label="Name" autoFocus isLoading={templateQuery.fetching} />
        ...
        <BannerField name="banner" initialImage={templateQuery.data?.template.banner} />
