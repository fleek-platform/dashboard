...
export const createExtraValidation = {
        ...
        return {
          ...
          message: (
            <>
              ...
              <ExternalLink href={`${constants.EXTERNAL_LINK.ENS_DOMAIN}/${name}/register`} colorScheme="yellow">
  ...
  siteName: (client: Client, gitProvider?: GitProvider) => async (siteName: string) => {
    try {
      if (gitProvider) {
        const isRepoNameAvailable = await gitProvider.isRepositoryNameAvailable({ repositoryName: siteName });

        if (!isRepoNameAvailable) {
          return { status: 'invalid', message: `A repository named ${siteName} already exists on your git provider` };
        }
