...
export type ComingSoonOverlayProps = ChildrenProps<LoadingProps<{ title?: string; description: string; className?: string }>>;
...
export const ComingSoonOverlay: React.FC<ComingSoonOverlayProps> = ({
  ...
  return (
    <Box onClick={(event) => event.stopPropagation()} className={cn('relative block', className)}>
      {children}
