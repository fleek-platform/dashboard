...
export const generatePaginationItems = ({
  totalPages,
  selectedPage,
}: GeneratePaginationItemsArgs): PaginationItem[] => {
  ...
  const isSideEllipsisDisplayRequired =
    selectedPage >= 4 && selectedPage <= totalPages - 3;
  ...
  const paginationItems = lengthArray.reduce<PaginationItem[]>(
    (acc, _, currentIndex) => {
      ...
      if (isSmallNumberOfPages || page === selectedPage) {
        ...
        const shouldDisplayEllipsis =
          ...
          (currentIndex === 0 || currentIndex === totalPages - 2);
